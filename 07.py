...
all_directories = [root] # I could do the tree traversal, but I'm not at a job interview
for line in data:
    if line == "$ ls":
        continue
    if line == "$ cd ..":
    ...
    if new_current_directory_name := re.search(r'\$ cd ([a-z]+)', line):
        current_node = next((x for x in current_node.children if x.name == new_current_directory_name.group(1)), None)
        if not current_node:
            print(line)
        continue
    ...
    if file_description := re.search(r'([0-9]+) ([a-z\.]+)', line):
        current_node.add_child(FileSystemNode(name=file_description.group(2), size=int(file_description.group(1))))
        continue
    raise ValueError(f"String pattern '{line}' cannot be recognized")
print(sum([node.get_size() for node in all_directories if node.get_size() <= 100000]))
space_to_clear = 30000000 - (70000000 - root.get_size())
print(min([node.get_size() for node in all_directories if node.get_size() >= space_to_clear]))
